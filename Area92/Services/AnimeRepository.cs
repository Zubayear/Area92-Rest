...
namespace Area92.Services
{
    public class AnimeRepository : IAnimeRepository
    {
        ...
        public AnimeRepository(AnimeContext animeContext, IPropertyMappingService propertyMappingService)
        {
            _animeContext = animeContext ?? throw new ArgumentNullException(nameof(animeContext));
        ...
        {
            return await _animeContext.Anime.FindAsync(id);
        }

        public async Task<IEnumerable<Anime>> GetAllAnime()
        {
            return await _animeContext.Anime.ToListAsync();
        }

        public async Task<PagedList<Anime>> GetAllAnime(AnimesResourceParameter animeResourceParameter)
        {
            var animes = _animeContext.Anime as IQueryable<Anime>;
        ...
        {
            return await _animeContext.Anime.Where(a => ids.Contains(a.Id)).ToListAsync();
        }
